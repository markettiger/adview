...
function getRangeLabel() {
  ...
  return `${map[state.rangeType]} (${start} ~ ${end})`;
}
...
function renderKpis() {
  ...
  ui.kpiImpressions.textContent = totals.impressions.toLocaleString();
  ui.kpiClicks.textContent = totals.clicks.toLocaleString();
  ui.kpiConversions.textContent = totals.conversions.toLocaleString();
  ui.kpiSpend.textContent = formatCurrency(totals.spend);
}
...
function renderSummary() {
  ui.rangeSummary.textContent = `선택 기간: ${getRangeLabel()}`;
  ui.lastUpdated.textContent = state.lastUpdatedAt ? `마지막 갱신: ${state.lastUpdatedAt}` : '마지막 갱신: 없음';
}
...
async function refreshData() {
  ...
  try {
    ...
    state.lastUpdatedAt = formatDateTime(new Date());
